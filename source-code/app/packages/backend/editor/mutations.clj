...
(ns app.packages.backend.editor.mutations
...
(defn add-item-f
  ...
  (if (x.user/request->authenticated? request)
      (let [prepare-f #(common/added-document-prototype request %)]
           (mongo-db/save-document! "packages" item {:prepare-f prepare-f}))))
...
(defmutation add-item!
             ...
             ; @return (namespaced map)
             [env mutation-props]
             {::pathom.co/op-name 'packages.editor/add-item!}
...
(defmutation save-item!
             ...
             ; @return (namespaced map)
             [env mutation-props]
             {::pathom.co/op-name 'packages.editor/save-item!}
