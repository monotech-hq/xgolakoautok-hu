...
(ns app.packages.backend.lister.mutations
...
(defn delete-items-f
  [_ {:keys [item-ids]}]
  (mongo-db/remove-documents! "packages" item-ids))

(defmutation delete-items!
             [env mutation-props]
             {::pathom.co/op-name 'packages.lister/delete-items!}
...
(defn undo-delete-items-f
  [_ {:keys [items]}]
  (mongo-db/insert-documents! "packages" items))

(defmutation undo-delete-items!
             [env mutation-props]
             {::pathom.co/op-name 'packages.lister/undo-delete-items!}
...
(defn duplicate-items-f
  ...
  (let [prepare-f #(common/duplicated-document-prototype request %)]
       (mongo-db/duplicate-documents! "packages" item-ids {:prepare-f prepare-f})))

(defmutation duplicate-items!
             [env mutation-props]
             {::pathom.co/op-name 'packages.lister/duplicate-items!}
