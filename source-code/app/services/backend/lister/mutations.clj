...
(ns app.services.backend.lister.mutations
...
(defn delete-items-f
  [_ {:keys [item-ids]}]
  (mongo-db/remove-documents! "services" item-ids))

(defmutation delete-items!
             [env mutation-props]
             {::pathom.co/op-name 'services.lister/delete-items!}
...
(defn undo-delete-items-f
  [_ {:keys [items]}]
  (mongo-db/insert-documents! "services" items))

(defmutation undo-delete-items!
             [env mutation-props]
             {::pathom.co/op-name 'services.lister/undo-delete-items!}
...
(defn duplicate-items-f
  ...
  (let [prepare-f #(common/duplicated-document-prototype request %)]
       (mongo-db/duplicate-documents! "services" item-ids {:prepare-f prepare-f})))

(defmutation duplicate-items!
             [env mutation-props]
             {::pathom.co/op-name 'services.lister/duplicate-items!}
