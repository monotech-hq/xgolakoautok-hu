...
(ns app.services.backend.editor.mutations
...
(defn add-item-f
  ...
  (let [prepare-f #(common/added-document-prototype request %)]
       (mongo-db/save-document! "services" item {:prepare-f prepare-f})))
...
(defmutation add-item!
             ...
             ; @return (namespaced map)
             [env mutation-props]
             {::pathom.co/op-name 'services.editor/add-item!}
...
(defmutation save-item!
             ...
             ; @return (namespaced map)
             [env mutation-props]
             {::pathom.co/op-name 'services.editor/save-item!}
