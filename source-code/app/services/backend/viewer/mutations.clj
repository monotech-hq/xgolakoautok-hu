...
(ns app.services.backend.viewer.mutations
...
(defn delete-item-f
  ...
  [_ {:keys [item-id]}]
  (mongo-db/remove-document! "services" item-id))
...
(defmutation delete-item!
             ...
             ; @return (namespaced map)
             [env mutation-props]
             {::pathom.co/op-name 'services.viewer/delete-item!}
...
(defn undo-delete-item-f
  ...
  ; @return (namespaced map)
  [_ {:keys [item]}]
  (mongo-db/insert-document! "services" item))
...
(defmutation undo-delete-item!
             ...
             ; @return (namespaced map)
             [env mutation-props]
             {::pathom.co/op-name 'services.viewer/undo-delete-item!}
...
(defn duplicate-item-f
  ...
  (let [prepare-f #(common/duplicated-document-prototype request %)]
       (mongo-db/duplicate-document! "services" item-id {:prepare-f prepare-f})))
...
(defmutation duplicate-item!
             ...
             ; @return (string)
             [env mutation-props]
             {::pathom.co/op-name 'services.viewer/duplicate-item!}
