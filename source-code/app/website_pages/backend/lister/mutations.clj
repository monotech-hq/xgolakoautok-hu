...
(ns app.website-pages.backend.lister.mutations
...
(defn delete-items-f
  ...
  (if (x.user/request->authenticated? request)
      (mongo-db/remove-documents! "website_pages" item-ids)))
...
(defmutation delete-items!
             ...
             ; @return (strings in vector)
             [env mutation-props]
             {::pathom.co/op-name 'website-pages.lister/delete-items!}
...
(defn undo-delete-items-f
  ...
  (if (x.user/request->authenticated? request)
      (mongo-db/insert-documents! "website_pages" items)))
...
(defmutation undo-delete-items!
             ...
             ; @return (namespaced maps in vector)
             [env mutation-props]
             {::pathom.co/op-name 'website-pages.lister/undo-delete-items!}
...
(defn duplicate-items-f
  ...
  (if (x.user/request->authenticated? request)
      (let [prepare-f #(common/duplicated-document-prototype request %)]
           (mongo-db/duplicate-documents! "website_pages" item-ids {:prepare-f prepare-f}))))
...
(defmutation duplicate-items!
             ...
             ; @return (keywords in vector)
             [env mutation-props]
             {::pathom.co/op-name 'website-pages.lister/duplicate-items!}
