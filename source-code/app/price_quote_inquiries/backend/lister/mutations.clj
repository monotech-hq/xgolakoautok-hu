...
(defn delete-items-f
  [_ {:keys [item-ids]}]
  (mongo-db/remove-documents! "price_quote_inquiries" item-ids))

(defmutation delete-items!
             [env mutation-props]
             {::pathom.co/op-name 'price-quote-inquiries.lister/delete-items!}
...
(defn undo-delete-items-f
  [_ {:keys [items]}]
  (mongo-db/insert-documents! "price_quote_inquiries" items))

(defmutation undo-delete-items!
             [env mutation-props]
             {::pathom.co/op-name 'price-quote-inquiries.lister/undo-delete-items!}
...
(defn duplicate-items-f
  ...
  (let [prepare-f #(common/duplicated-document-prototype request %)]
       (mongo-db/duplicate-documents! "price_quote_inquiries" item-ids {:prepare-f prepare-f})))

(defmutation duplicate-items!
             [env mutation-props]
             {::pathom.co/op-name 'price-quote-inquiries.lister/duplicate-items!}
