...
(ns app.categories.backend.editor.mutations
...
(defn add-item-f
  ...
  (let [prepare-f #(common/added-document-prototype request %)]
       (mongo-db/save-document! "categories" item {:ordered? true :prepare-f prepare-f})))
...
(defmutation add-item!
             ...
             ; @return (namespaced map)
             [env mutation-props]
             {::pathom.co/op-name 'categories.editor/add-item!}
...
(defmutation save-item!
             ...
             ; @return (namespaced map)
             [env mutation-props]
             {::pathom.co/op-name 'categories.editor/save-item!}
