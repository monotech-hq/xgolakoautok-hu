...
(ns app.types.backend.lister.mutations
...
(defn delete-items-f
  [_ {:keys [item-ids]}]
  (mongo-db/remove-documents! "types" item-ids))

(defmutation delete-items!
             [env mutation-props]
             {::pathom.co/op-name 'types.lister/delete-items!}
...
(defn undo-delete-items-f
  [_ {:keys [items]}]
  (mongo-db/insert-documents! "types" items))

(defmutation undo-delete-items!
             [env mutation-props]
             {::pathom.co/op-name 'types.lister/undo-delete-items!}
...
(defn duplicate-items-f
  ...
  (let [prepare-f #(common/duplicated-document-prototype request %)]
       (mongo-db/duplicate-documents! "types" item-ids {:prepare-f prepare-f})))

(defmutation duplicate-items!
             [env mutation-props]
             {::pathom.co/op-name 'types.lister/duplicate-items!}
