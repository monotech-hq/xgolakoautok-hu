...
(defn get-net-total-price
  ; @return (integer)
  []
  (let [vehicle-unit-price      @(r/subscribe [:db/get-item [:price-quotes :viewer/viewed-item :vehicle-unit-price]])
        vehicle-unique-price    @(r/subscribe [:db/get-item [:price-quotes :viewer/viewed-item :vehicle-unique-price]])
        vehicle-unique-pricing? @(r/subscribe [:db/get-item [:price-quotes :viewer/viewed-item :vehicle-unique-pricing?]])
        packages-price          @(r/subscribe [:db/get-item [:price-quotes :viewer/viewed-item :packages-price]])
        products-price          @(r/subscribe [:db/get-item [:price-quotes :viewer/viewed-item :products-price]])
