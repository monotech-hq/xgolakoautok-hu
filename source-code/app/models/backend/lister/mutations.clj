...
(ns app.models.backend.lister.mutations
...
(defn delete-items-f
  [_ {:keys [item-ids]}]
  (mongo-db/remove-documents! "models" item-ids))

(defmutation delete-items!
             [env mutation-props]
             {::pathom.co/op-name 'models.lister/delete-items!}
...
(defn undo-delete-items-f
  [_ {:keys [items]}]
  (mongo-db/insert-documents! "models" items))

(defmutation undo-delete-items!
             [env mutation-props]
             {::pathom.co/op-name 'models.lister/undo-delete-items!}
...
(defn duplicate-items-f
  ...
  (let [prepare-f #(common/duplicated-document-prototype request %)]
       (mongo-db/duplicate-documents! "models" item-ids {:prepare-f prepare-f})))

(defmutation duplicate-items!
             [env mutation-props]
             {::pathom.co/op-name 'models.lister/duplicate-items!}
