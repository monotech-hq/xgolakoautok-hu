...
(ns app.vehicle-models.backend.editor.mutations
...
(defn add-item-f
  ...
  (if (x.user/request->authenticated? request)
      (let [prepare-f #(common/added-document-prototype request %)]
           (mongo-db/save-document! "vehicle_models" item {:prepare-f prepare-f}))))
...
(defmutation add-item!
             ...
             ; @return (namespaced map)
             [env mutation-props]
             {::pathom.co/op-name 'vehicle-models.editor/add-item!}
...
(defmutation save-item!
             ...
             ; @return (namespaced map)
             [env mutation-props]
             {::pathom.co/op-name 'vehicle-models.editor/save-item!}
